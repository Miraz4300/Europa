...
<?php if ($rs['cover']): ?>
<style>.de-restaurantspage .de-restaurant-head:before {background-image: url(<?=path.'/'.$rs['cover']?>)}</style>
<?php endif; ?>
<div class="container">
	<div class="de-restaurant-head">
		<div class="de-dtable">
			<div class="de-vmiddle">
				<div class="de-profile">
					<div class="de-thumb">
					...
					<div class="de-name"><h1><?=$rs['name']?></h1></div>
					<div class="de-stars"><?php echo fh_stars($rs['id'], "restaurant") ?></div>
					<div class="de-address"><i class="fas fa-map-marker-alt"></i> <?=$rs['address']?></div>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="row">
				...
				<div class="tab-content">
					<div class="tab-pane active container" id="home">
						...
						while($rs_m = $sql->fetch_assoc()):
							$i++;
						?>
							<div class="de-resaurant<?=($i==1?' open':'')?>">
								<h3 class="de-title<?=($i==1?' active':'')?>">
									...
									<div class="de-resaurant">
										<div class="media">
											<div class="media-left">
												<div class="de-thumb"><img src="<?=path?>/<?=$item_rs['image']?>" alt="<?=$item_rs['name']?>" onerror="this.src='<?=noimage?>'"></div>
											</div>
												...
												<div class="de-dtable">
													<div class="de-vmiddle">
														...
														<div class="de-options">
															...
															<a class="de-out"><?=$lang['restaurant']['out']?></a>
															<?php endif; ?>
															<?php if($item_rs['reduce_price'] && $item_rs['reduce_price'] != $item_rs['selling_price']): ?>
															<a class="de-reduce"><?=dollar_sign.$item_rs['reduce_price']?></a>
															<?php endif; ?>
															<?php if ($rs['neworders']): ?>
															<div class="de-addtocart" data-id="<?=$item_rs['id']?>">
																<a data-toggle="modal" href="#addtocartModal" class="de-addtobasket tips"><i class="fas fa-shopping-basket"></i><b><?=$lang['home']['addtocart']?></b></a>
																<a data-toggle="modal" href="#addtocartModal" class="de-price"><?=dollar_sign.$item_rs['selling_price']?></a>
															</div>
															<?php else: ?>
																<a class="de-reduce text-success"><?=dollar_sign.$item_rs['selling_price']?></a>
															<?php endif; ?>
														</div>
													</div>
												</div>
					...
					<div class="tab-pane container" id="menu2">
						...
						?>
						<div class="col-3">
						<div class="de-album-img"><a href="<?=path.'/'.$rs_img['url']?>" data-lightbox="image-1"><img src="<?=path.'/'.$rs_img['url']?>" onerror="this.src='<?=noimage?>'"></a></div>
						</div>
						<?php
						endwhile;
						else:
		...
		<div class="col-4">
			<div class="de-restaurant-sidebar">
				<div class="de-search">
					<div class="de-input">
...
<div class="container">
	<div class="row de-restaurants">
		...
		<div class="col-4">
			<div class="de-item">
				<div class="de-thumb"><img src="<?=path?>/<?=$rs['photo']?>" alt="<?=$rs['name']?>" onerror="this.src='<?=noimage?>'"></div>
				<div class="de-title"><a href="<?=path?>/restaurants.php?id=<?=$rs['id']?>&t=<?=fh_seoURL($rs['name'])?>"><h3><?=$rs['name']?></h3></a></div>
				<div class="de-stars"><?php echo fh_stars($rs['id'], "restaurant") ?></div>
				<div class="de-address"><i class="fas fa-map-marker-alt"></i> <?=$rs['city']?> - <?=$countries[$rs['country']]?></div>
			</div>
		</div>
		<?php endwhile; ?>
		<?php else: ?>
