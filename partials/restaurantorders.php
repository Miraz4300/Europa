			...
			<div class="de-dtable">
				<div class="de-vmiddle">
					<div class="de-title">
						<h3>
							<a href="#">
								<?=$rsi['name']?><?php if($item_size): ?><strong class="de-color"> (<?=$item_size['name']?>)</strong><?php endif; ?>
							</a>
						</h3>
					</div>
					<div class="de-extra">
					...
					<div class="de-extra">
						...
						?>
							<span>
								<?=$extra['name']?> <b class="de-extraprice"><?=dollar_sign.$extra['price']?></b>
							</span>
						<?php
						endforeach;
						endif;
					...
					<div class="de-options">
						<?php if($rs['status']==2): ?>
						<a class="de-delivered wdth"><i class="fas fa-check"></i> <?=$lang['myorders']['delivered']?></a>
						<?php elseif($rs['status']==1): ?>
						<a class="de-awaiting wdth bg-v"><i class="fas fa-truck"></i> <?=$lang['myorders']['intheway']?></a>
						<?php else: ?>
						<a class="de-awaiting wdth"><i class="fas fa-clock"></i> <?=$lang['myorders']['awaiting']?></a>
						<?php endif; ?>
						<?php if($rs['status']!=2): ?>
						<a href="#" class="de-delivered tips" data-id="<?php echo $rs['id'] ?>"><i class="fas fa-check"></i><b><?=$lang['myorders']['make_it_delivered']?></b></a>
						<?php endif; ?>
						<?php if($rs['status']==0): ?>
						<a href="#" class="de-intheway tips" data-id="<?php echo $rs['id'] ?>"><i class="fas fa-truck"></i><b><?=$lang['myorders']['intheway']?></b></a>
						<?php endif; ?>
						<?php if (fh_access('invoices')): ?>
						<a href="#" class="de-addinvoice tips" data-name="<?php echo time()."_".fh_seoURL($rsi['name']); ?>" data-id="<?php echo $rs['id']; ?>"><i class="fas fa-file-invoice-dollar"></i><b><?=$lang['myorders']['invoice']?></b></a>
						<?php endif; ?>
					</div>
			...
			<table class="table border">
				...
				<tbody>
					<tr>
						...
						<td class="left strong"><?=$rsi['name']?><?php if($item_size): ?><strong class="de-color"> (<?=$item_size['name']?>)</strong><?php endif; ?></td>
