	...
	<div class="media-left">
		...
		<div class="media-body">
			...
			<div class="de-quantity">
				<i class="fas fa-minus de-minus"></i>
				<input type="text" name="item_quantity" value="1" disabled>
				<i class="fas fa-plus de-plus"></i>
			</div>
			<?php if($item_rs['sizes']): ?>
			<div class="de-sizes form-inline">
				<?php
				$x = 0;
			...
			<span class="de-ingredient"><i class="fas fa-utensils"></i> <b>Ingredient:</b> <?=$item_rs['ingredients']?></span>
			<textarea name="item_note" placeholder="Your extra note"></textarea>
		</div>
</div>
<div class="de-price-det">
	<div class="de-price de-totalprice"><?=dollar_sign.$item_rs['selling_price']?></div>
	<?php if($item_rs['reduce_price'] && $item_rs['reduce_price'] != $item_rs['selling_price']): ?>
	<div class="de-reduce"><?=dollar_sign.$item_rs['reduce_price']?></div>
	<?php endif; ?>
	<button type="button" class="de-btn de-buytocart" data-buy="false">Add to cart</button>
	<button type="submit" class="de-buy" data-buy="true">Buy now</button>
</div>
...
<div class="tab-content">
		...
		<table class="table">
			<?php foreach(unserialize($item_rs['extra']) as $k => $v): ?>
				<tr>
					...
					<td>
						<?=dollar_sign.$v['price']?>
						<a href="#" class="de-addexratoitem" data-price="<?=$v['price']?>" data-id="<?=$k?>"><i class="fas fa-plus-circle"></i></a>
					</td>
				</tr>
			<?php endforeach; ?>
		</table>
	...
	<div class="tab-pane<?=(!$item_rs['extra']?' active':'')?> container" id="menus1">
		<div class="de-desc"><?=nl2br($item_rs['description'])?></div>
	</div>
	<?php endif; ?>
