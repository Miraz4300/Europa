...
<div class="container">
	...
	<div class="row">
		...
		<div class="de-cart-body">
			...
			<div class="de-cart-body">
				<div class="media">
					<div class="media-left"><div class="de-thumb"><img src="<?=path?>/<?=$rs['image']?>" alt="<?=$rs['name']?>" onerror="this.src='<?=noimage?>'"></div></div>
					<div class="media-body">
						<div class="de-dtable">
							<div class="de-vmiddle">
								<div class="de-title"><h3><a href="#"><?=$rs['name']?></a></h3></div>
								<div class="de-extra"><small><?=str_replace("+", " ", $cart['item_note'])?></small></div>
								<?php if ($rs['delivery_price']): ?>
								<div class="d-none de-deliveryprice"><?=$rs['delivery_price']?></div>
								<?php endif; ?>
								<div class="de-extra">
									<?php if($item_size): ?>
								...
								<div class="de-extra">
									...
									?>
										<span>
											<?=$extra['name']?> <b class="de-extraprice"><?=$extra['price']?></b>
											<i class="fas fa-times de-removeexratoitem" data-price="<?=$extra['price']?>" data-id="<?=$k?>" data-iid="<?=$it_id?>" data-iiid="<?=$cart_id?>"></i>
										</span>
									<?php
									endforeach;
									endif;
								...
								<div class="de-options">
									<a href="#" class="de-removefromcart" data-iid="<?=$it_id?>" data-iiid="<?=$cart_id?>"><i class="fas fa-trash"></i></a>
									<div class="de-quantity">
										<i class="fas fa-minus de-minus" data-i="<?=$it_id?>" data-a="<?=$cart_id?>"></i>
										<input type="text" name="item_quantity" value="<?=$cart['item_quantities']?>" disabled>
										<i class="fas fa-plus de-plus" data-i="<?=$it_id?>" data-a="<?=$cart_id?>"></i>
									</div>
								</div>
			...
			<div class="de-cart-body">
		      ...
		      <form id="payment-form" method="POST" action="stripe.php">
						...
						<div class="fieldset mb-4">
							<p class="mt-2"><span><?=$lang['cart']['subtotal']?></span><b class="float-right de-totalprice"><?=dollar_sign?>0.00</b></p>
							<p><span><?=$lang['cart']['shipping_fees']?></span><b class="float-right de-deliverytotalprice"><?=dollar_sign?>0.00</b></p>
							<hr />
							<p><span><?=$lang['cart']['total']?></span><b class="float-right de-alltotalprice"><?=dollar_sign?>0.00</b></p>
						</div>
