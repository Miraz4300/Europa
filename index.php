...
<div class="de-section de-best">
	<div class="container">
		...
		if($nearby->num_rows):
		 ...
		 <div class="col-3">
			 <div class="de-item">
				 <div class="de-thumb"><img src="<?=path?>/<?=$nearby_rs['photo']?>" alt="<?=$nearby_rs['name']?>" onerror="this.src='<?=noimage?>'"></div>
				 <div class="de-title"><a href="<?=path?>/restaurants.php?id=<?=$nearby_rs['id']?>&t=<?=fh_seoURL($nearby_rs['name'])?>"><h3><?=$nearby_rs['name']?></h3></a></div>
				 <div class="de-stars"><?php echo fh_stars($nearby_rs['id'], "restaurant") ?></div>
				 <div class="de-address"><i class="fas fa-map-marker-alt"></i> <?=$nearby_rs['city']?> - <?=$countries[$nearby_rs['country']]?></div>
			 </div>
		 </div>
		 <?php endwhile; ?>
		 <?php else: ?>
			...
			<div class="col-4">
				<div class="de-post">
					...
					<div class="de-details">
						...
						<div class="de-title"><h1><a href="<?=path?>/restaurants.php?id=<?=$rs['restaurant']?>&t=<?=fh_seoURL($rs['rname'])?>"><?=$rs['name']?></a></h1></div>
						<div class="de-info">
		...
		<div class="de-testimonials">
			<div class="owl-carousel owl-theme">
				...
				<div class="de-item">
					...
					<div class="de-content"><?=$rs['content']?></div>
					<div class="de-stars">
					...
					<div class="de-author"><?=db_get("users", "username", $rs['author'])?></div>
				</div>
				</div>
				<?php endwhile; ?>
				<?php $sql->close(); ?>
