<div class="de-breadcrumb">
	...
	<div class="de-options">
		<a href="<?=path?>/dashboard.php?pg=items&request=new" class="de-btn"><i class="fas fa-plus"></i> Create an Item</a>
	</div>
</div>

<?php if ($request == "new"): ?>
	<div class="de-box p-5">
	...
	<div class="de-resaurant">
		...
		<table class="table">
			...
			<tbody>
				...
				?>
				<tr>
					<th width="1"><div class="de-thumb m-0"><img src="<?=path?>/<?=$rs['image']?>" onerror="this.src='<?=noimage?>'"></div></th>
					<td width="40%">
					...
					<td class="de-dot-options">
						...
						<ul class="de-drop">
							<li><a href="#" class="de-itemhome" data-id="<?=$rs['id']?>"><i class="fas fa-home"></i> <?=$lang['dash']['athome']?></a></li>
							<li><a href="<?=path?>/dashboard.php?pg=items&request=new&id=<?=$rs['id']?>"><i class="far fa-edit"></i> Edit</a></li>
							<li><a href="#" class="de-delete" data-type="item" data-id="<?=$rs['id']?>"><i class="fas fa-trash-alt"></i> Delete</a></li>
						</ul>
					</td>
				</tr>
				<?php endwhile; ?>
